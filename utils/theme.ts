...
export const shiftHSL = (
	...
	const [h, s, l] = splitHSL(hsl) as [number, number, number];
...
export type BreakpointOverrides = Partial<
...
const indexByBreakpoint: Record<BreakpointKey, number> = {
	mobileSmall: 1,
	mobileLarge: 2,
	tablet: 3,
	desktopSmall: 4, // SD
...
export const mediaQueryLessThan = (
	...
	const allDefaults: (string | number | undefined)[] = [
		defaultValue, // Offset?
		defaultValue,
		defaultValue,
		defaultValue,
	...
	if (!lessThanIndex || lessThanValue === undefined || lessThanValue === null) {
		return allDefaults;
	}
...
export type StandardColorKey = keyof typeof standardColors;

export const getStandardColors = (colorProp: string): string =>
	standardColors[colorProp as StandardColorKey] ?? PRIMARY_COLOR;
