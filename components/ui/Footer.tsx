...
const LeafNavigationBox = styled(Box)<{ reducedHeight?: boolean }>`
  padding-left: 24px;
  position: relative;
...
const ConnectLink = ({ href, icon }: ConnectLinkProps) => (
	<a href={href}>
		<ConnectIcon>
	...
	<FooterContainer>
		<Flex width={1} pb={4} justify="space-between">
			...
			>
				...
				<Flex mt={3} column>
					<Box mr={4} my={3} onClick={scrollIntoView("/home")}>
						<NavigationText medium>{"Home"}</NavigationText>
					</Box>
					<Box
						mr={4}
						my={3}
						onClick={scrollIntoView("/my-work", { block: "start" })}
					>
						<NavigationText medium>{"My Work"}</NavigationText>
					</Box>
					<LeafNavigationBox
						mr={4}
						my={3}
						onClick={scrollIntoView("/computer-graphics", { block: "start" })}
					>
						<NavigationText medium>{"Computer Graphics"}</NavigationText>
					</LeafNavigationBox>
					<LeafNavigationBox
						mr={4}
						my={3}
						onClick={scrollIntoView("/web-development", { block: "start" })}
					>
						<NavigationText medium>{"Web Development"}</NavigationText>
					</LeafNavigationBox>
					<LeafNavigationBox
						mr={4}
						my={3}
						reducedHeight
						onClick={scrollIntoView("/photography", { block: "start" })}
					>
						<NavigationText medium>{"Photography"}</NavigationText>
					</LeafNavigationBox>
					<Box
						mr={4}
						my={3}
						onClick={scrollIntoView("/about", { block: "start" })}
					>
						<NavigationText medium>{"About"}</NavigationText>
					</Box>
					<Box
						mr={4}
						my={3}
						onClick={scrollIntoView("/contact", { block: "start" })}
					>
						<NavigationText medium>{"Contact Me"}</NavigationText>
					</Box>
					<Box
						mr={4}
						my={3}
						as="a"
						href="https://blog.williamgermany.com"
						target="_blank"
						rel="noopener noreferrer"
						style={{ textDecoration: "none" }}
					>
						<NavigationText medium>{"Blog"}</NavigationText>
					</Box>
				</Flex>
			</Flex>
			<Flex width={0.5} column>
				...
				>
					<Flex>
						<ConnectLink
							href="https://twitter.com/WillGermany"
							icon="twitter"
						/>
						<ConnectLink
							href="https://instagram.com/_ice.photography_"
							icon="instagram"
						/>
		...
		>
			<Box my={mediaQueryLessThan("0px", "mobileLarge", "8px")}>
				<a
					href="https://github.com/willywill/site"
					style={{ textDecoration: "none" }}
				>
					<UnderlineText color={SUBTITLE_COLOR} letterSpacing={0.3}>
